<x-os-layout>
    ...
    <div class="space-y-4">
        <div class="bg-white rounded-xl shadow-sm p-5 flex flex-wrap items-center justify-between gap-3">
            ...
            @if ($user->canAccess('jobs') && \Illuminate\Support\Facades\Gate::allows('create', \App\Models\Job::class))
                <a href="{{ route('jobs.create') }}" class="text-sm font-semibold px-4 py-2 rounded-md bg-[#E91E63] text-white hover:opacity-90">+ New job</a>
            @endif
        </div>
        ...
        @if ($dueJobs->isNotEmpty() || $queueCount > 0)
            <div class="space-y-2">
                @foreach ($dueJobs as $job)
                    <a href="{{ route('jobs.show', $job) }}" class="flex items-center gap-3 rounded-lg bg-red-50 text-red-700 text-sm px-4 py-2.5 hover:bg-red-100">
                        <span>⚠️</span>
                ...
                @endforeach
                @if ($queueCount > 0)
                    <a href="{{ route('jobs.index', ['view' => 'queue']) }}" class="flex items-center gap-3 rounded-lg bg-amber-50 text-amber-800 text-sm px-4 py-2.5 hover:bg-amber-100">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                @if ($user->canAccess('jobs'))
                    ...
                    <div class="mt-3 flex-1 divide-y divide-gray-100 text-sm">
                        @forelse ($myJobs->take(5) as $job)
                            ...
                            <a href="{{ route('jobs.show', $job) }}" class="flex items-center justify-between gap-2 py-2 hover:bg-gray-50">
                    ...
                    <div class="mt-4 flex flex-wrap gap-2">
                        <a href="{{ route('dashboard') }}" class="text-xs font-semibold px-3 py-1.5 rounded-md border border-gray-200 hover:bg-gray-50">Open Jobs</a>
                        <a href="{{ route('jobs.index', ['view' => 'queue']) }}" class="text-xs font-semibold px-3 py-1.5 rounded-md border border-gray-200 hover:bg-gray-50">Queue{{ $queueCount ? " ({$queueCount})" : '' }}</a>
                    </div>
                @else
            ...
            <div class="bg-white rounded-xl shadow-sm p-5 flex flex-col {{ $user->canAccess('finance') ? '' : 'opacity-50' }}">
                ...
                @if ($user->canAccess('finance'))
                    ...
                    <div class="mt-4"><a href="{{ route('finance.index') }}" class="text-xs font-semibold px-3 py-1.5 rounded-md border border-gray-200 hover:bg-gray-50">Open Finance</a></div>
                @else
                    <p class="text-sm text-gray-500 mt-2">No access. Ask BOD if you need it.</p>
                @endif
            </div>
